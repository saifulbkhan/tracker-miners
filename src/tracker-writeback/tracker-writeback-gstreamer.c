...
static void
pad_added_cb (GstElement *decodebin, GstPad *pad, TagElements *element)
{
...
make_temp (const gchar *uri, gchar **uri_final, GOutputStream **stream, GError **io_error)
{
	...
	if (basename == NULL) {
		g_object_unref (file);
		g_set_error (io_error, G_IO_ERROR, G_IO_ERROR_FAILED, "Could not find basename for file: %s", uri);
		return FALSE;
	}
	...
	if (parent == NULL) {
		g_object_unref (file);
		g_free (basename);
		g_set_error (io_error, G_IO_ERROR, G_IO_ERROR_FAILED, "Could not find parent for file: %s", uri);
		return FALSE;
	}
	g_object_unref (file);

	dotted = g_strdup_printf (".%s", basename);
	g_free (basename);
	...
	hidden_uri = g_file_get_uri (file);
	g_object_unref (file);

	do {
		prefixed_uri = g_strdup_printf ("%s%06X", hidden_uri, g_random_int_range (0, 0xFFFFFF));

		file = g_file_new_for_uri (prefixed_uri);
		fstream = g_file_create (file, G_FILE_CREATE_PRIVATE, NULL, &err);
		if (err != NULL) {
		...
		g_object_unref (file);
	} while (err == NULL && fstream == NULL);

	g_free (hidden_uri);

	if (fstream != NULL) {
		*uri_final = prefixed_uri;
...
static void
writeback_gstreamer_save (TagElements *element, const gchar *uri, GError **error)
{
	...
	make_temp (uri, &tmpname, &stream, &io_error);
	if (io_error != NULL) {
		goto gio_error;
	}
 ...
 out:
	if (tmpname != NULL) {
		g_debug ("removing temporary file");
		dest = g_file_new_for_uri (tmpname);
		if (g_file_delete (dest, NULL, NULL) == FALSE) {
			g_debug ("unable to delete temporary file");
		}
		g_object_unref (dest);
	}
	g_free (tmpname);
...
static gboolean
writeback_gstreamer_set (TagElements *element, const gchar *tag, const GValue *val)
{
                                          ...
                                          GError                 **error)
{
	...
	for (n = 0; n < values->len; n++) {
		...
		if (g_strcmp0 (row[2], TRACKER_PREFIX_NIE "comment") == 0) {
			g_value_init (&val, G_TYPE_STRING);
			g_value_set_string (&val, row[3]);
			writeback_gstreamer_set (element, GST_TAG_EXTENDED_COMMENT, &val);
		}
...
static gchar *
writeback_gstreamer_get_artist_name (TrackerSparqlConnection *connection, const gchar *urn)
{
	gchar *val, *query;
...
static gchar *
writeback_gstreamer_get_album_name (TrackerSparqlConnection *connection, const gchar *urn)
{
	gchar *val, *query;
...
static gchar *
writeback_gstreamer_get_album_artist (TrackerSparqlConnection *connection, const gchar *urn)
{
...
static gchar *
writeback_gstreamer_get_disc_number (TrackerSparqlConnection *connection, const gchar *urn)
{
	gchar *val, *query;
...
static gchar *
writeback_gstreamer_get_publisher_name (TrackerSparqlConnection *connection, const gchar *urn)
{
	gchar *val, *query;
